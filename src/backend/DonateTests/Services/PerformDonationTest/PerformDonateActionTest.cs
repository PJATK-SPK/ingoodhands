...
namespace DonateTests.Services.PerformDonationTest
{
    ...
    public class PerformDonateActionTest
    {
        ...
        [TestMethod()]
        public async Task PerformDonateActionTest_NoRole_ThrowsException()
        {
            using var toolkit = new TestsToolkit(_usedModules);
            var context = toolkit.Resolve<AppDbContext>();
            var action = toolkit.Resolve<PerformDonateAction>();
            ...
            //Act 
            var exception = await Assert.ThrowsExceptionAsync<UnauthorizedException>(() => action.Execute(donatePayload));
