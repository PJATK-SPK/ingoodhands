...
namespace AuthService.BusinessLogic.GetCurrentUser
{
    public class GetCurrentUserService
    {
        ...
        {
            var userFromDatabase = await _appDbContext.Auth0Users.Where(c => c.Email == auth0UserInfo.Email).Select(c => c.User).SingleOrDefaultAsync();
            if (userFromDatabase == null)
            {
                _logger.LogError("UserFromDatabase is null");
